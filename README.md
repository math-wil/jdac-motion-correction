...
The next scientific direction documented in `research-notes` is to strengthen the fidelity evaluation, especially with test-retest ICC, and to compare an external ready-to-run method such as BME-X before deciding whether modifying or retraining JDAC is justified.
...
6. Run the phase 4 notebook or scripts to regenerate the comparison tables.
