...
**Avancement du 24 septembre :** les 9/9 extractions et les 9/9 inférences sont terminées sur le PC labo ; les montages et la géométrie ont été contrôlés. Sur Narval, les **18/18 reconstructions FreeSurfer** possèdent `aseg.stats`, `lh.aparc.stats` et `rh.aparc.stats`. Le job initial `3850218` en avait produit 17 ; `preproc_minimal/sub-17_run-01`, interrompu par le quota de fichiers du groupe `def-sbouix`, a été repris sous le groupe du projet `ctb-sbouix` par le job ciblé `3904648` (terminé avec code 0). Le contrôle qualité des segmentations et la comparaison morphométrique restent à faire.
