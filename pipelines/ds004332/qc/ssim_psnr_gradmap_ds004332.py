"""
SSIM et PSNR sur gradient maps avant/après correction JDAC pour ds004332.
Métrique anti-artefact du papier JDAC (Zhang et al., arXiv 2403.08162), tableau 1 colonne droite.

Gradient map : |∇I| = sqrt(gx² + gy² + gz²), normalisé [0,1].
Référence    : run-01 (sans mouvement, espace FLIRT)
...
def gradmap(img: np.ndarray) -> np.ndarray:
    gx, gy, gz = np.gradient(img)
    g = np.sqrt(gx**2 + gy**2 + gz**2)
    vmax = g.max()
    return (g / vmax).astype(np.float32) if vmax > 1e-8 else g
...
for sub in SUBS:
    ...
    if not ref_path.exists():
        ...
        continue
    ref_g = gradmap(nib.load(ref_path).get_fdata(dtype=np.float32))
    ...
    for run in ("run-02", "run-03"):
        ...
        if not before_path.exists() or not after_path.exists():
            print(f"[SKIP] {sub} {run} : fichier manquant")
            continue

        before_g = gradmap(nib.load(before_path).get_fdata(dtype=np.float32))
        after_g  = gradmap(nib.load(after_path).get_fdata(dtype=np.float32))

        if ref_g.shape != before_g.shape or ref_g.shape != after_g.shape:
            print(f"[SKIP] {sub} {run} : shapes incompatibles")
            continue

        ssim_before = ssim(ref_g, before_g, data_range=1.0)
        psnr_before = psnr(ref_g, before_g, data_range=1.0)
        ssim_after  = ssim(ref_g, after_g,  data_range=1.0)
        psnr_after  = psnr(ref_g, after_g,  data_range=1.0)
